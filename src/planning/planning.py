...
# set up goal state
# read data
demo_path = './data/folding/0c795a2e62284cf28fffd048ad58b769.pkl' # target demo path
demo_data = load_gzip_file(demo_path)
demo_states = demo_data['states']
demo_actions = demo_data['actions']
demo_actions = np.stack(demo_actions).squeeze(1) # (50, 26)
...
# load model
config_path = './config/folding.yaml'
...
model_path = 'ckpts/folding.pth' # model checkpoint path
model.eval()
...
actions = nn.Parameter(torch.randn(50, 26).to(device))
# actions = nn.Parameter(torch.from_numpy(demo_actions).float().to(device))

# optimize
optim = torch.optim.Adam([actions], lr=1e-2)
ran = tqdm.trange(50)
for i in ran:
    optim.zero_grad()
    ...
    # loss = torch.nn.functional.mse_loss(pred, torch.from_numpy(goal_points).float().to(device))
    loss.backward()
    optim.step()
